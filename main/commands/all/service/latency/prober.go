...
func executeRun(cmd *base.Command, args []string) {
	...
	wg, err := startWorkers(ctx, wnum, upstream)
	if err != nil {
		mlog.Error(err)
		cancel()
		return
	}
	go watchSingalTerm(mlog, cancel)
	wg.Wait()
	mlog.Info("prober stopped")
}

func startWorkers(ctx context.Context, wnum int, upstream *Upstream) (*sync.WaitGroup, error) {
	if wnum < 1 {
...
func watchSingalTerm(mlog *Logger, cancel context.CancelFunc) {
	...
	for {
		...
		default:
			...
			if err != nil {
				...
				wlog.Warn("recv shutdown message")
				return
			} else if !resp.Ok {
